...
<body>
                    ...
                    <div class="col-md-5 mt-1">
                        ...
                        <select class="form-control" id="product_id">
                            ...
                            <?php foreach ($products as $key => $value): ?>
                                ...
                                <option value="{{$value->ProductID}}" data-purchasePrice={{$value->unit_sale_price}} data-taxPercentage="{{$value->taxPercentage}}" data-allowPercentage="{{$value->allow_percentage ?? 0}}">{{$value->ProductName}} | PS: {{$value->pack_size}} | Qty {{$value->avaliable_qty}}</option>
                                @endif
                            <?php endforeach; ?>
                        </select>
                ...
                <div class="footer-section">
                    <div class="row">
                        ...
                        <div class="col-md-2">
                            ...
                            <input type="number" id="invoice_discount" style="color:red;font-weight: bold; font-size: 14px;" value="0" class="form-control">
                        </div>
                        <div class="col-md-2">
    ...
    <script type="text/javascript">
        ...
        function validateAndApplyDiscount() {
            ...
            ProductList.forEach(function(item) {
                var allowPercentage = parseFloat(item.allow_percentage) || 0;
        ...
        function reload_table() {
            ...
            ProductList.forEach((value, key) => {
                // Calculate quantities after returns
                var actualQuantity = parseFloat(value.Quantity) - parseFloat(value.ReturnQuantity || 0);

                // Bill Amount = (quantity - returnquantity) * UnitePrice (before discount)
                var itemTotal = actualQuantity * parseFloat(value.UnitePrice);
                value.Total = itemTotal; // Update the Total to reflect actual quantity

                var discountAmount = 0;

                // If quantity after return is zero, then discount_percentage_amount = 0
                if (actualQuantity <= 0) {
                    discountAmount = 0;
                    value.discount_percentage_amount = 0;
                } else if (value.discount_percentage > 0) {
                    discountAmount = (itemTotal * value.discount_percentage) / 100;
                    value.discount_percentage_amount = discountAmount;
                } else {
                    value.discount_percentage_amount = 0;
                }

                var finalItemAmount = itemTotal - discountAmount;

                var html = `<tr>
                        <td style="width: 5%" data-id="${value.ProductID}">${key+1}</td>
                        <td style="width: 20%">${value.Name}</td>

                        <td style="width: 10%" class="editable" data-field="quantity">${actualQuantity}</td>
                        <td style="width: 10%" class="" data-field="rate">${value.UnitePrice}</td>
                        <td style="width: 10%">${finalItemAmount.toFixed(2)}</td>
                ...
                total_amount = parseFloat(total_amount) + finalItemAmount + parseFloat(value.taxAmount);
            ...
            // Bill Amount = before discount (quantity - returnquantity) * UnitePrice
            $("#BillAmount").val(Math.ceil(total_amount + total_discount_amount));
            $("#ReceivedAmount").val(0);
            // Balance is After discount = Bill Amount - Product Discounts - Invoice Discount
            $("#PatientBalance").val(Math.ceil(total_amount - invoice_discount));
            //$("#ReceivedAmount").val(0);
