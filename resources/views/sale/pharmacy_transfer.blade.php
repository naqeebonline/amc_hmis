...
<div class="container-fluid">
    ...
    <div class="row header-section">
        ...
        <div class="col-md-4">
            ...
            <input type="text" id="previous_balance" class="form-control" value="Transfer by {{auth()->user()->name}}" style="font-weight: bold;" >
        </div>
    ...
    <div class="row mt-2">
                        ...
                        <div class="col-md-5 mt-1">
                            ...
                            <select class="form-control" id="product_id">
                                ...
                                <?php foreach ($products as $key => $value): ?>
                                    ...
                                    <option value="{{$value->ProductID}}" data-purchasePrice={{$value->PurchasePrice}} data-taxPercentage="{{$value->taxPercentage}}">{{$value->ProductName}} | PS: {{$value->pack_size}} | Qty {{$value->avaliable_qty}}</option>
                                    @endif
                                <?php endforeach; ?>
                            </select>
        ...
        <div class="col-md-4 sidebar">
            ...
            {{--<table class="table table-bordered">
                ...
                <h5 style="text-align: center; color:green">Pharmacy Transfer</h5>
...
<script>
    ...
    function calculateReturnAmount(){
     ...
     previous_bill_table = $('#previous-bill-table').DataTable({
                ...
                pageLength: 8,
                ajax: {
                    url: `{{ route('pos.pharmacy_transfer_bills') }}`,
        ...
        $("body").on("click","#save_bill",function(){
           ...
           var patient_id = SID;
            $.ajax({
                ...
                url: "{{ route("pos.save_pharmacy_transfer") }}",
                success:function(response){
                    $("#save_bill").show();
                    sale_id_for_print=response.id;
                    setTimeout(function () {
                        window.location.reload();
                        //window.location="{{route('pos.pharmacy_transfer')}}";
                    },1500);


                    url="{{route('pos.print_pharmacy_transfer_bill')}}/"+sale_id_for_print;
